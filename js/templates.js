const Templates = {
    ...
    questionReferral(argsEn) {
        if (argsEn.referralEn !== null) {
            const discoverEn = 'file=discover.pdf';
            const hostname = `http://${window.location.host}/`;
            const link = `<a href="${hostname}pdfviewer/web/viewer.html?${discoverEn}#${argsEn.referralEn}" target="_blank">page #${argsEn.referralEn}</a>`;
